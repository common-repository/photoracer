...
$frompostid=isset($_REQUEST['postid'])?$_REQUEST['postid']:null;
if ($frompostid==null){
	$frompostid=isset($_REQUEST['prid'])?$_REQUEST['prid']:null;
}
...
function retrieve_password() {
	...
	$message .= get_option('siteurl') . "/wp-content/plugins/photoracer/prwp-login.php?action=rp&key=$key\r\n";
...
function reset_password($key) {
	...
	$message .= get_option('siteurl') . "/wp-content/plugins/photoracer/prwp-login.php?postid=$frompostid\r\n";
...
case 'logout' :
	...
	$redirect_to = 'prwp-login.php?loggedout=true&postid='.$frompostid;
...
case 'retrievepassword' :
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			wp_redirect('prwp-login.php?checkemail=confirm&postid='.$frompostid);
			exit();
		}
	}
...
<div class="titolo_pag"><h1><?php echo $out->name?></h1></div>
	<div class="pagina">
		<div class="pagina-int">
			<div id="content" class="narrowcolumn">
<form name="lostpasswordform" id="lostpasswordform" action="prwp-login.php?action=lostpassword&postid=<?php echo $frompostid; ?>" method="post">
	<p>
...
<a href="<?php bloginfo('wpurl'); ?>/wp-content/plugins/photoracer/prwp-login.php?postid=<?php echo $frompostid; ?>"><?php _e('Log in') ?></a> |
<a href="<?php bloginfo('wpurl'); ?>/wp-content/plugins/photoracer/prwp-login.php?action=register&postid=<?php echo $frompostid; ?>"><?php _e('Register') ?></a>
<?php else : ?>
<a href="<?php bloginfo('wpurl'); ?>/wp-content/plugins/photoracer/prwp-login.php?postid=<?php echo $frompostid; ?>"><?php _e('Log in') ?></a>
<?php endif; ?>
</p>
...
<p id="backtoblog"><a href="<?php bloginfo('url'); ?>/" title="<?php _e('Are you lost?') ?>">
<?php printf(__('&laquo; Back to %s'), get_bloginfo('title', 'display' )); ?></a></p>
</div>
</div>
</div>
<?php get_sidebar(); ?>
...
case 'rp' :
	...
	if ( ! is_wp_error($errors) ) {
		wp_redirect('prwp-login.php?checkemail=newpass&postid='.$frompostid);
		exit();
	}
	
	wp_redirect('prwp-login.php?action=lostpassword&error=invalidkey&postid='.$frompostid);
	exit();
...
case 'register' :
	if ( !get_option('users_can_register') ) {
		wp_redirect('prwp-login.php?registration=disabled&postid='.$frompostid);
		exit();
	}
	...
	if ( $http_post ) {
		...
		$user_meta_name = $_POST['user_meta_name'];
		$user_meta_surname = $_POST['user_meta_surname'];
		
		$ok_metaname = 1;
		$ok_metasurname = 1;
		
		if ($user_meta_name == null || 
			$user_meta_name == "" || 
			preg_match('/\d+/',$user_meta_name) || 
			strlen($user_meta_name) < 3){
			$errors->add('empty_username', __('<strong>ERROR</strong>: Il nome &egrave; obbligatorio.'));
			$ok_metaname = 0;
		}
		if ($user_meta_surname == null || 
			$user_meta_surname == "" || 
			preg_match('/\d+/',$user_meta_surname) || 
			strlen($user_meta_surname) < 3){
			$errors->add('empty_username', __('<strong>ERROR</strong>: Il cognome &egrave; obbligatorio.'));
			$ok_metasurname = 0;
		}
		if ($ok_metaname && $ok_metasurname) {
			
			$errors = register_new_user($user_login, $user_email);
			if ( !is_wp_error($errors) ) {
				$userobj = get_user_by_email($user_email);
				update_usermeta($userobj->ID,'first_name',$user_meta_name);
				update_usermeta($userobj->ID,'last_name',$user_meta_surname);
				wp_redirect('prwp-login.php?checkemail=registered&postid='.$frompostid);
				exit();
			} 
		}
...
<form name="registerform" id="registerform" action="prwp-login.php?action=register" method="post">
	<p>
		<label>Nome<br />
		<input type="text" name="user_meta_name" id="user_meta_name" class="input" value="<?php echo attribute_escape(stripslashes($user_meta_name)); ?>" size="25" tabindex="20" /></label>
	</p>
	<p>
		<label>Cognome<br />
		<input type="text" name="user_meta_surname" id="user_meta_surname" class="input" value="<?php echo attribute_escape(stripslashes($user_meta_surname)); ?>" size="25" tabindex="20" /></label>
	</p>
	<p>
	...
	<input type="hidden" name="postid" value="<?php echo $frompostid; ?>">
...
<p id="nav">
<a href="<?php bloginfo('wpurl'); ?>/wp-content/plugins/photoracer/prwp-login.php?postid=<?php echo $frompostid; ?>"><?php _e('Log in') ?></a> |
<a href="<?php bloginfo('wpurl'); ?>/wp-content/plugins/photoracer/prwp-login.php?action=lostpassword&postid=<?php echo $frompostid; ?>" title="<?php _e('Password Lost and Found') ?>"><?php _e('Lost your password?') ?></a>
</p>
...
default:
	if ( isset( $_REQUEST['redirect_to'] ) )
		...
		$redirect_to = '/?page_id='.$frompostid;
	...
	if ( !is_wp_error($user) ) {
		...
		if (isset($frompostid)){
			$redirect_to = get_option('siteurl') . '/?page_id='.$frompostid;
		}
		elseif ( !$user->has_cap('edit_posts') && ( empty( $redirect_to ) || $redirect_to == 'wp-admin/' ) )
...
<form name="loginform" id="loginform" action="prwp-login.php" method="post">
